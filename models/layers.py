...
class LearnableEnsembleWeight(Layer):
    """可学习的集成权重层

    通过softmax学习每个基础模型的最优权重。
    输入: (batch, num_models) — 各模型的预测值拼接
    输出: (batch, 1) — 加权平均预测值
    """
    ...
    def get_weights_vector(self):
        """返回学习到的softmax归一化权重"""
        return tf.nn.softmax(self.raw_weights).numpy()

    def get_config(self):
        config = super().get_config()
        config.update({"num_models": self.num_models})
        return config
