"""
集成模型
支持加权平均和堆叠集成
"""
import tensorflow as tf
from tensorflow.keras import Model, layers
import numpy as np
from typing import List, Optional

from models.base_model import BaseModel
from models.initializers import get_initializer
from models.layers import LearnableEnsembleWeight
from config.model_config import EnsembleConfig, DEFAULT_ENSEMBLE_CONFIG


class EnsembleModel(BaseModel):
    """多模型集成"""

    def __init__(
        self,
        base_models: List[BaseModel],
        config: EnsembleConfig = DEFAULT_ENSEMBLE_CONFIG,
        name: str = "ensemble_model",
        task_type: str = "classification"
    ):
        super().__init__(config, name, task_type)
        self.base_models = base_models
        self.config = config

    def build_model(self):
        if len(self.base_models) == 0:
            raise ValueError("No base models provided")

        for model in self.base_models:
            if model.model is None:
                model.build_model()

        inputs = layers.Input(shape=self.base_models[0].config.input_shape)

        predictions = []
        for i, model in enumerate(self.base_models):
            pred = model.model(inputs)
            predictions.append(pred)

        if len(predictions) == 1:
            outputs = predictions[0]
        else:
            if self.config.use_weighted_average:
                if self.config.learnable_weights:
                    # 可学习权重：concat各模型预测 → softmax加权
                    concatenated = layers.Concatenate(axis=-1)(predictions)
                    self.weight_layer = LearnableEnsembleWeight(
                        num_models=len(predictions),
                        name="learnable_weights"
                    )
                    outputs = self.weight_layer(concatenated)
                else:
                    weights = self.config.weights
                    if weights is None:
                        weights = [1.0 / len(predictions)] * len(predictions)

                    weighted_preds = []
                    for pred, weight in zip(predictions, weights):
                        weighted_pred = layers.Lambda(lambda x: x * weight)(pred)
                        weighted_preds.append(weighted_pred)

                    outputs = layers.Add()(weighted_preds)
            elif self.config.use_stacking:
                concatenated = layers.Concatenate()(predictions)
                x = layers.Dense(
                    self.config.stacking_units,
                    activation="relu",
                    kernel_initializer=get_initializer("xavier", seed=42)
                )(concatenated)
                if self.task_type == "classification":
                    outputs = layers.Dense(
                        1,
                        activation="sigmoid",
                        kernel_initializer=get_initializer("xavier", seed=42)
                    )(x)
                else:
                    outputs = layers.Dense(
                        1,
                        kernel_initializer=get_initializer("xavier", seed=42)
                    )(x)
            else:
                outputs = layers.Average()(predictions)

        self.model = Model(inputs=inputs, outputs=outputs, name=self.name)
        return self.model

    def predict_ensemble(self, X: np.ndarray) -> np.ndarray:
        predictions = []
        for model in self.base_models:
            pred = model.predict(X)
            predictions.append(pred)
        return np.array(predictions)

    def get_learned_weights(self) -> Optional[np.ndarray]:
        """返回学习到的集成权重（仅当learnable_weights=True时有效）"""
        if hasattr(self, 'weight_layer') and self.weight_layer is not None:
            return self.weight_layer.get_weights_vector()
        return None
